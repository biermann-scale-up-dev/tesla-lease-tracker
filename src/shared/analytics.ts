...
export function summarize(contract: Contract, preferences: Preferences, input: readonly Sample[], at: string): Summary {
  ...
  const todayTrips = trips.filter(trip => trip.end !== null && localDate(trip.end, contract.timeZone) === today);
  const totals = { todayRecordedKm: todayTrips.reduce((sum, trip) => sum + (trip.distanceKm ?? 0), 0), todayTrips: todayTrips.length, completeTrips: trips.filter(trip => trip.status === 'complete').length, incompleteTrips: trips.filter(trip => trip.status !== 'complete').length };
  return { totals, asOf: at, trackingSince: samples[0]?.at ?? null, odometerAt: last?.at ?? null, odometerKm, usedKm, allowedKm, remainingKm, deviationKm: usedKm === null ? null : usedKm - allowedKm, dailyBudgetKm: remainingKm === null || remainingDays === 0 ? null : remainingKm / remainingDays, remainingDays, elapsedDays, totalDays, unassignedKm: Math.max(0, knownTravel - assignedTravel), ended, trips: [...trips].reverse(), gaps, daily, forecasts };
}
