...
- Node.js, React, Vite, Fastify, TypeScript, Recharts, KafkaJS, node-qrcode (MIT), Zod, the Temporal polyfill, Lucide and other npm dependencies retain the license files shipped in their packages. `package-lock.json` records the exact dependency versions.
- Scriptable is a separately installed iOS application with its own terms. Only the original MIT-licensed integration script is distributed here.
